        ...
        <motion.div variants={item} className="space-y-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-sm text-slate-500 font-medium mt-1">Cross-sectional reporting for {format(new Date(dateRange.start), 'MMM d')} — {format(new Date(dateRange.end), 'MMM d, yyyy')}</p>
            </div>
            <div className="flex items-center gap-3 bg-white p-2 rounded-2xl shadow-sm border border-slate-100">
               <div className="flex flex-col px-3 border-r border-slate-100">
                  <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Start Date</span>
                  <Input 
                    type="date" 
                    value={dateRange.start} 
                    onChange={e => setDateRange({...dateRange, start: e.target.value})}
                    className="h-7 border-none p-0 text-xs font-bold text-slate-900 focus-visible:ring-0 w-28 bg-transparent" 
                  />
               </div>
               <div className="flex flex-col px-3">
                  <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">End Date</span>
                  <Input 
                    type="date" 
                    value={dateRange.end} 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="premium-card p-8 h-[400px] flex flex-col">
              <div className="mb-6">
                <h3 className="font-bold text-slate-900 text-lg tracking-tight">Patient Volume Trends</h3>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Intake growth over selected range</p>
              </div>
              <div className="flex-1 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={reports?.volume || []}>
                    <defs>
            ...
            <div className="premium-card p-8 h-[400px] flex flex-col">
              <div className="mb-6">
                <h3 className="font-bold text-slate-900 text-lg tracking-tight">Doctor Workload Analysis</h3>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Comparison of patients handled per physician</p>
              </div>
              <div className="flex-1 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={reports?.workload || []} layout="vertical">
                    ...
                    <Tooltip 
                      ...
                      contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontSize: '12px'}}
                    />
                    <Bar dataKey="patients" fill="#4f46e5" radius={[0, 4, 4, 0]} barSize={20} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
